...
public class Baralho {
	...
	while(true){
            ...
            if(indice == 4)	break;		//se o indice chegar a 4 quer dizer que ja adicionou todas as cores de carta (0 ~ 3)
        
            novaCartaNumero = new CartaNumero(cor[indice], j, corCarta[indice] + "" + j + ".jpg");
            baralho.push(novaCartaNumero);
            if(j != 0)baralho.push(novaCartaNumero);	//0 s� possui 1 unidade
            j++;
	}
		
        novaCartaEspecial = new CartaEspecial("Preto", "+4", "+4.jpg");
        for (int i = 0; i < 4; i++) baralho.push(novaCartaEspecial);
